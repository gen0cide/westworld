...
var Events = []EventSpec{
	...
	{Name: "equipment_changed", Params: []string{"slot", "item"},
		DocSummary: "Our OWN worn/wielded set changed. Fires once per changed slot: slot is the human slot name (helmet/body/legs/weapon/...), item is the new worn item there (item.is_empty if removed). Also read self.equipped / self.equipped.bonuses for the full new state. (A bare `on equipment_changed()` still works — extra args are ignored.)"},
