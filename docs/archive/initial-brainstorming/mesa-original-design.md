> **ARCHIVED (initial brainstorming), 2026-06-10.** This is the original REST/SQL/TrustGrade
> design for mesa. The service manifested differently: a **gRPC** service (`mesa/proto/mesa.proto`
> — Game/Knowledge/Journal/KV/Provision/Admin) over Postgres + pgvector (`mesa/mesad/ltm.go`),
> with the consolidation/insight crons (`mesa/mesad/cron.go`, `cron_insight.go`) instead of the
> five background jobs designed here, and `cmd/mesa-ctl` as the operator plane. The REST
> endpoints, the bots/routines/routine_versions/persona_revisions/brain_calls/knowledge_chunks/
> bot_vocabulary/working_scratch tables, and the `MockClient` never manifested; the **TrustGrade
> trust-authority model REVERSED** — trust is host-local (`limbic/ledger.go`, AuthLocal in
> `memory/policy.go`) and mesa only mirrors it (see the decision record in `docs/mesa.md`,
> rewritten same-commit). Still-open items were harvested into `docs/TODO.md` (M-7, O-9).
