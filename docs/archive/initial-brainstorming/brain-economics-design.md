...
> differently. What shipped: three fixed per-tier LLM seams mesa-side (`mesa/mesad/server.go`
> `actLLM`/`decideLLM`/`genesisLLM` over the SDK-free `mesa/llm` client), the Act
> author→validate→re-prompt loop with mesa-side DSL validation (`mesa/mesad/act.go`),
> prompt-caching of the shared DSL manual via `llm.SystemBlock` (`mesa/llm/anthropic.go`,
> `mesa/mesad/dslmanual.go`), and the host-side `brain.Strategist` seam with the
> `StubStrategist` offline fallback (`brain/`). The living doc is `docs/brain.md` (rewritten
> same-commit). Never shipped from the body below: `AnthropicBrain` + `DecisionClass`-keyed
> routing/limiters, the `brain_calls` cost ledger + per-host budgets (open as **O-9** in
> `docs/TODO.md`), the `DSLGrammar`/`DSLSurface` earned-vocabulary split (operator call,
> **C-12**), `MockBrain`, and the mesa-free tutorial DIRECT bootstrap mode (the host completed
> tutorial island through the standard mesa Act loop instead). Kept verbatim below for the
> record.

# Brain economics + earned-vocabulary design (Phase-4 `AnthropicBrain`)

These sections are the original Phase-4 design body of `docs/brain.md`. The invocation
taxonomy (when the brain is consulted, and the four-way decision union) stayed in the living
doc — it manifested as `mesapb.MoveKind`. Everything below did not.
