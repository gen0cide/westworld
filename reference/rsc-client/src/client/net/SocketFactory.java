...
import client.Mudclient;             // client — the GameData loader writes the equipSlotJk table (obf client.Jk)
...
public abstract class SocketFactory {
    ...
    /**
     * View-frustum far-Z accumulator (obf {@code m.j}; clean m.java:16 {@code static int j}).
     * One of the six unified frustum-bound globals the obfuscator scattered across
     * {@code da/m/oa/aa/nb}: written by {@code Scene} (obf {@code lb}) and read by
     * {@code GameModel.project()} (obf {@code ca}). Companion to {@link client.net.ClientStream#frustumNearZ}.
     */
     ...
     * <ul>
     *   <li>{@code InputState.nameDataBuffer} (obf {@code kb.d}) — decoded string archive bytes
     ...
     *       <li>{@code DecodeBuffer.chatFilterCache} (obf {@code ac.x}) — item name strings
     *           (pre-filled with empty-string sentinels via {@code ISAAC.readString()}
     *            / obf {@code o.a(byte 38)}; the byte guard was dropped from the deob sig).</li>
     *       <li>{@code IntHolder.scratchInts} (obf {@code ka.c}) — members-only flag (1 = members).</li>
     *       <li>{@code TextEncoder.scratchIntArray2} (obf {@code h.c}) — item tint colors.</li>
     *       <li>{@code Scene.diagStrings} (obf {@code lb.ac}) — item examine strings.</li>
     *       <li>{@code Surface.unusedIntsBb} (obf {@code ua.Bb}) — item sprite widths.</li>
     *       <li>{@code InputState.pixelData} (obf {@code kb.b}) — item flags.</li>
     *       <li>{@code Utility.k} (obf {@code mb.k}) — item unknown-K values.</li>
     *       <li>{@code InputState.itemC} (obf {@code kb.c}) — item team/category (1 = stackable).</li>
     *       <li>{@code CharTable.itemDescriptions} (obf {@code ga.b}) — item examine strings (alternate).</li>
     *       <li>{@code ProxySocketFactory._deadIntArray} (obf {@code gb.s}) — item ground-sprite flags.</li>
     *     </ul>
     *   </li>
    ...
    public static final void initGameData(byte[] rawData, byte param, boolean dummy) {
        ...
        // z[7] = "string.dat", key byte -124; result stored in InputState.nameDataBuffer (kb.d)
        // obf na.a(String,int,byte[],int) is declared StreamFactory.lookupEntityDefRecord
        // (the trailing keyByte int was dropped from the deob 3-arg signature).
        InputState.nameDataBuffer = StreamFactory.lookupEntityDefRecord(
            /* key: */ "string.dat", /* z[7] */
            /* offset: */ 0,
            rawData
        );
        ...
        DownloadWorker.unusedP = 0; // obf: jb.p = 0  (reset the string-data read cursor; declared DownloadWorker.unusedP)

        // z[4] = "integer.dat", key byte -125; result stored in Packet.legacyBytes (b.v)
        // obf na.a(...) is declared StreamFactory.lookupEntityDefRecord (keyByte int dropped).
        Packet.legacyBytes = StreamFactory.lookupEntityDefRecord(
            /* key: */ "integer.dat", /* z[4] */
            /* offset: */ 0,
            rawData
        );
        ...
        // advances this cursor as it consumes bytes from Packet.legacyBytes (b.v); without this
        ...
        // ProxySocketFactory._deadAntitamperFlag = EntityDef.getCount(65525)  (obf: gb.p = t.a(65525))
        int itemCount = EntityDef.getCount(65525); // obf: gb.p = t.a(65525)
        ProxySocketFactory._deadAntitamperFlag = itemCount;

        // Allocate all per-item arrays (size = itemCount)
        ClientIOException.itemY          = new int[itemCount];         // obf: fa.e
        DecodeBuffer.chatFilterCache           = new String[itemCount];      // obf: ac.x
        IntHolder.scratchInts            = new int[itemCount];         // obf: ka.c (members flag; ==1 → members-only)
        TextEncoder.scratchIntArray2           = new int[itemCount];         // obf: h.c
        Scene.diagStrings           = new String[itemCount];      // obf: lb.ac (Scene.diagStrings slot)
        Surface.unusedIntsBb                   = new int[itemCount];         // obf: ua.Bb
        InputState.pixelData             = new int[itemCount];         // obf: kb.b
        Utility.k                    = new int[itemCount];         // obf: mb.k
        InputState.slotFlags          = new int[itemCount];         // obf: kb.c
        CharTable.itemDescriptions              = new String[itemCount];      // obf: ga.b
        ProxySocketFactory._deadIntArray   = new int[itemCount];         // obf: gb.s

        // Pre-fill string arrays with empty-string sentinels
        // obf: o.a(byte 38) is ISAAC.readString() — reads a NUL-terminated string from the shared buffer
        for (int i = 0; i < itemCount; i++) {
            DecodeBuffer.chatFilterCache[i] = ISAAC.readString(); // obf: o.a((byte)38)
        }
        for (int i = 0; i < itemCount; i++) {
            CharTable.itemDescriptions[i] = ISAAC.readString();
        }
        for (int i = 0; i < itemCount; i++) {
            Scene.diagStrings[i] = ISAAC.readString();
        }

        // Fill per-item numeric arrays from decoded archives
        for (int i = 0; i < itemCount; i++) {
            Surface.unusedIntsBb[i] = EntityDef.readSpriteId(65525); // obf: ua.Bb[i] = t.a(65525)
            // Track the global sprite count maximum
            if (Utility.chatLineState < Surface.unusedIntsBb[i] + 1) { // obf: mb.l
                Utility.chatLineState = Surface.unusedIntsBb[i] + 1;
            }
        }
        for (int i = 0; i < itemCount; i++) {
            InputState.pixelData[i] = NameTable.readUnsignedInt((byte) -105); // obf: ub.a
        }
        for (int i = 0; i < itemCount; i++) {
            ClientIOException.itemY[i] = ChatCipher.readNextBufferByte(); // obf: v.a(-30504)
        }
        for (int i = 0; i < itemCount; i++) {
            ProxySocketFactory._deadIntArray[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < itemCount; i++) {
            Utility.k[i] = EntityDef.readSpriteId(65525);
        }
        for (int i = 0; i < itemCount; i++) {
            TextEncoder.scratchIntArray2[i] = NameTable.readUnsignedInt((byte) -105);
        }
        ...
        for (int i = 0; i < itemCount; i++) {
            InputState.slotFlags[i] = ChatCipher.readNextBufferByte(); // obf: kb.c[i]
        }
        for (int i = 0; i < itemCount; i++) {
            IntHolder.scratchInts[i] = ChatCipher.readNextBufferByte(); // obf: ka.c[i] (members flag)
        }
        ...
        for (int i = 0; i < itemCount; i++) {
            if (/* not: dummy==1 */ true && IntHolder.scratchInts[i] == 1) {
                DecodeBuffer.chatFilterCache[i]     = STRINGS[6]; // "Members object"
                CharTable.itemDescriptions[i]        = STRINGS[5]; // "You need to be a member to use this object"
                InputState.pixelData[i]       = 0;
                Scene.diagStrings[i]     = "";
                ProxySocketFactory._deadIntArray[0] = 0;  // Note: index 0, not i — matches bytecode
                Utility.k[i]              = 0;
                InputState.slotFlags[i]    = 1;
            }
        }
        ...
        SurfaceImageProducer.entityIndexTableD     = new int[entityCount];    // obf: fb.d
        Packet.legacyMaskTable                         = new int[entityCount];    // obf: b.h
        DownloadWorker.unusedIntArray                 = new int[entityCount];    // obf: jb.k
        ArchiveReader.sectorAlloc                  = new int[entityCount];    // obf: ob.h
        ClientRuntimeException.intScratch         = new int[entityCount];    // obf: la.a (la = ClientRuntimeException)
        itemSpriteIndex                       = new int[entityCount];    // obf: m.g
        ChatCipher.unusedE                     = new int[entityCount];    // obf: v.e
        ISAAC.unusedA                          = new int[entityCount];    // obf: o.a  (note: conflicts with o.a static method — field)
        SurfaceSprite.equipAc                 = new String[entityCount]; // obf: ba.ac
        SurfaceImageProducer.entityIndexTableC           = new int[entityCount];    // obf: fb.c
        Timer.legacyStringsE                          = new String[entityCount]; // obf: p.e
        ClientStream.sharedIntArrayT                   = new int[entityCount];    // obf: da.T
        GameShell.equipMb                     = new String[entityCount]; // obf: e.Mb
        StreamFactory.deadGuardArray                  = new int[entityCount];    // obf: na.a
        LinkedQueue.sharedIntArray                    = new int[entityCount];    // obf: db.j
        GameFrame.unusedIntBuffer2d                      = new int[entityCount][12]; // obf: qb.d  — 12-slot equipment grid
        AudioMixer.scratchBuffer                     = new int[entityCount];    // obf: eb.b
        Surface.unusedIntsAb                       = new int[entityCount];    // obf: ua.Ab

        // Pre-fill String arrays with empty sentinels
        for (int i = 0; i < entityCount; i++) {
            GameShell.equipMb[i] = ISAAC.readString();
        }
        for (int i = 0; i < entityCount; i++) {
            SurfaceSprite.equipAc[i] = ISAAC.readString();
        }

        // Fill per-entity numeric arrays
        for (int i = 0; i < entityCount; i++) {
            ClientRuntimeException.intScratch[i] = ChatCipher.readNextBufferByte(); // obf: la.a[i]
        }
        for (int i = 0; i < entityCount; i++) {
            AudioMixer.scratchBuffer[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < entityCount; i++) {
            SurfaceImageProducer.entityIndexTableD[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < entityCount; i++) {
            DownloadWorker.unusedIntArray[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < entityCount; i++) {
            ISAAC.unusedA[i] = ChatCipher.readNextBufferByte();
        }

        // qb.d[i][j]: 12-slot equipment sprite grid per entity; value 255 → -1 (sentinel)
        for (int i = 0; i < entityCount; i++) {
            for (int j = 0; j < 12; j++) {
                GameFrame.unusedIntBuffer2d[i][j] = ChatCipher.readNextBufferByte();
                if (GameFrame.unusedIntBuffer2d[i][j] == 255) {
                    GameFrame.unusedIntBuffer2d[i][j] = -1; // 255 is the "no sprite" sentinel
                }
            }
        }

        for (int i = 0; i < entityCount; i++) {
            ClientStream.sharedIntArrayT[i] = NameTable.readUnsignedInt((byte) -105);
        }
        for (int i = 0; i < entityCount; i++) {
            itemSpriteIndex[i] = NameTable.readUnsignedInt((byte) -105); // obf: m.g[i] = ub.a(-105)
        }
        for (int i = 0; i < entityCount; i++) {
            Surface.unusedIntsAb[i] = NameTable.readUnsignedInt((byte) -105);
        }
        ...
        for (int i = 0; i < entityCount; i++) {
            ChatCipher.unusedE[i] = NameTable.readUnsignedInt((byte) -105);
        }
        for (int i = 0; i < entityCount; i++) {
            SurfaceImageProducer.entityIndexTableC[i] = EntityDef.readSpriteId(65525);
        }
        for (int i = 0; i < entityCount; i++) {
            Packet.legacyMaskTable[i] = EntityDef.readSpriteId(65525);
        }
        for (int i = 0; i < entityCount; i++) {
            ArchiveReader.sectorAlloc[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < entityCount; i++) {
            StreamFactory.deadGuardArray[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < entityCount; i++) {
            LinkedQueue.sharedIntArray[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < entityCount; i++) {
        ...
        for (int i = 0; i < npcCount; i++) {
            Utility.chatLines[i] = ISAAC.readString();
        }
        for (int i = 0; i < npcCount; i++) {
            Timer.legacyStringsC[i] = ISAAC.readString();
        }

        // --- Tier 3b: Prayer/spell tables ---
        int prayerCount = EntityDef.getCount(65525); // obf: na.e = t.a(65525)
        BZip.entityFlags         = new int[prayerCount];    // obf: aa.c (aa = BZip; NOT ArrayUtil/ab)
        CacheUpdater.contentNames = new String[prayerCount]; // obf: cb.e
        DataStore.tamperScratch    = new int[prayerCount];    // obf: nb.d
        FontWidths.prayerM   = new int[prayerCount];    // obf: n.m
        WorldEntity.spriteOffsets  = new int[prayerCount];    // obf: w.g
        LinkedQueue.sharedIntArray2  = new int[prayerCount];    // obf: db.l

        for (int i = 0; i < prayerCount; i++) {
            CacheUpdater.contentNames[i] = ISAAC.readString();
        }
        for (int i = 0; i < prayerCount; i++) {
            LinkedQueue.sharedIntArray2[i] = NameTable.readUnsignedInt((byte) -105);
        }
        for (int i = 0; i < prayerCount; i++) {
            FontWidths.prayerM[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < prayerCount; i++) {
            DataStore.tamperScratch[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < prayerCount; i++) {
            BZip.entityFlags[i] = ChatCipher.readNextBufferByte(); // obf: aa.c[i]
        }
        for (int i = 0; i < prayerCount; i++) {
            WorldEntity.spriteOffsets[i] = ChatCipher.readNextBufferByte();
        }

        // --- Tier 3c: Spell/ability tables ---
        int spellCount = EntityDef.getCount(65525); // obf: ua.Db = t.a(65525)
        Utility.sizedPoolCounts    = new int[spellCount];    // obf: mb.a
        NameTable.sortKeys  = new int[spellCount];    // obf: ub.g
        ClientRuntimeException.stringScratch = new String[spellCount]; // obf: la.f (la = ClientRuntimeException)
        Globals.paramNames    = new String[spellCount]; // obf: l.a  (l = Globals; NOT ListNode/g)
        RecordLoader.intArray = new int[spellCount];  // obf: f.f
        Timer.legacyStringsA      = new String[spellCount]; // obf: p.a
        FontBuilder.injectedStrings2 = new String[spellCount];// obf: s.f
        SurfaceImageProducer.entityIndexTableF = new int[spellCount]; // obf: fb.f
        TextEncoder.scratchIntArray1 = new int[spellCount];   // obf: h.b

        for (int i = 0; i < spellCount; i++) {
            Globals.paramNames[i] = ISAAC.readString(); // obf: l.a[i]
        }
        for (int i = 0; i < spellCount; i++) {
            ClientRuntimeException.stringScratch[i] = ISAAC.readString(); // obf: la.f[i]
        }
        for (int i = 0; i < spellCount; i++) {
            FontBuilder.injectedStrings2[i] = ISAAC.readString();
        }
        for (int i = 0; i < spellCount; i++) {
            Timer.legacyStringsA[i] = ISAAC.readString();
        }
        for (int i = 0; i < spellCount; i++) {
            // obf: fb.f[i] = ca.a((byte)91, o.a((byte)38))
            // GameModel.textureId(byte 91, ISAAC.readString()) — resolves/appends a
            // texture-name index (returns 0 for the empty/"Invisible" sentinel).
            SurfaceImageProducer.entityIndexTableF[i] = GameModel.textureId((byte) 91, ISAAC.readString());
        }
        for (int i = 0; i < spellCount; i++) {
            RecordLoader.intArray[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < spellCount; i++) {
            NameTable.sortKeys[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < spellCount; i++) {
            Utility.sizedPoolCounts[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < spellCount; i++) {
            TextEncoder.scratchIntArray1[i] = ChatCipher.readNextBufferByte();
        }

        // --- Tier 3d: Equipment-slot tables ---
        int equipSlotCount = EntityDef.getCount(65525); // obf: h.a = t.a(65525)
        StringCodec.DEAD_STRING_ARRAY  = new String[equipSlotCount]; // obf: u.b
        ...
        StreamBase._deadIntArray_d       = new int[equipSlotCount];    // obf: ib.d  (ib = StreamBase; NOT FilterStage/hb)

        for (int i = 0; i < equipSlotCount; i++) {
            GameCharacter.sharedNameTable[i] = ISAAC.readString();
        }
        for (int i = 0; i < equipSlotCount; i++) {
            NameTable.textureNames[i] = ISAAC.readString();
        }
        for (int i = 0; i < equipSlotCount; i++) {
            StringCodec.DEAD_STRING_ARRAY[i] = ISAAC.readString();
        }
        for (int i = 0; i < equipSlotCount; i++) {
            RecordLoader.stringTable[i] = ISAAC.readString();
        }
        for (int i = 0; i < equipSlotCount; i++) {
            StreamBase._deadIntArray_d[i] = EntityDef.readSpriteId(65525); // obf: ib.d[i]
        }
        for (int i = 0; i < equipSlotCount; i++) {
        ...
        for (int i = 0; i < equipSlotCount; i++) {
            Scene.diagScratch[i] = ChatCipher.readNextBufferByte();
        }

        // --- Tier 3e: Render-size tables ---
        int renderSizeCount = EntityDef.getCount(65525); // obf: v.h = t.a(65525)
        CacheFile.sharedScratch  = new int[renderSizeCount]; // obf: d.g
        ErrorHandler.unusedIntTable = new int[renderSizeCount]; // obf: i.g  (note: class i = ErrorHandler)

        for (int i = 0; i < renderSizeCount; i++) {
            ErrorHandler.unusedIntTable[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < renderSizeCount; i++) {
            CacheFile.sharedScratch[i] = ChatCipher.readNextBufferByte();
        }

        // --- Tier 3f: Texture tables ---
        int textureCount = EntityDef.getCount(65525); // obf: ia.h = t.a(65525)
        ClientStream.sharedIntArrayN  = new int[textureCount]; // obf: da.N
        DecodeBuffer.landscapeFaceFlags  = new int[textureCount]; // obf: ac.l
        Panel.texK         = new int[textureCount]; // obf: qa.K

        for (int i = 0; i < textureCount; i++) {
            Panel.texK[i] = NameTable.readUnsignedInt((byte) -105);
        }
        for (int i = 0; i < textureCount; i++) {
            ClientStream.sharedIntArrayN[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < textureCount; i++) {
            DecodeBuffer.landscapeFaceFlags[i] = ChatCipher.readNextBufferByte();
        }
        ...
        NameHash.idTable     = new int[mapEntityCount][];    // obf: oa.d
        GameFrame.unusedIntBuffer     = new int[mapEntityCount];      // obf: qb.e
        ClientStream.sharedIntTable2d        = new int[mapEntityCount][];    // obf: da.J
        BitBuffer.UNUSED_L  = new String[mapEntityCount];   // obf: ja.L
        NameHash.entryNames  = new String[mapEntityCount];   // obf: oa.a
        ImageLoader.sharedIntArray   = new int[mapEntityCount];      // obf: pa.f
        ISAAC.unusedP         = new int[mapEntityCount];      // obf: o.p

        for (int i = 0; i < mapEntityCount; i++) {
            BitBuffer.UNUSED_L[i] = ISAAC.readString();
        }
        for (int i = 0; i < mapEntityCount; i++) {
            NameHash.entryNames[i] = ISAAC.readString();
        }
        for (int i = 0; i < mapEntityCount; i++) {
            ImageLoader.sharedIntArray[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < mapEntityCount; i++) {
            ISAAC.unusedP[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < mapEntityCount; i++) {
            GameFrame.unusedIntBuffer[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < mapEntityCount; i++) {
            int subLen = ChatCipher.readNextBufferByte(); // obf: v.a(-30504)
            NameHash.idTable[i] = new int[subLen];
            for (int j = 0; j < subLen; j++) {
                // plain assignment — clean base has NO 255→-1 sentinel here (unlike equipD)
                NameHash.idTable[i][j] = EntityDef.readSpriteId(65525);
            }
        }
        for (int i = 0; i < mapEntityCount; i++) {
            int subLen = ChatCipher.readNextBufferByte();
            ClientStream.sharedIntTable2d[i] = new int[subLen];
            for (int j = 0; j < subLen; j++) {
                ClientStream.sharedIntTable2d[i][j] = ChatCipher.readNextBufferByte();
            }
        }

        // --- Tier 3h: Sound tables ---
        int soundCount = EntityDef.getCount(65525); // obf: t.g = t.a(65525)
        EntityDef.modelNames  = new String[soundCount]; // obf: t.h
        GameModel.sharedScratch      = new int[soundCount];    // obf: ca.B (GameModel.sharedScratch slot)
        ClientIOException.soundC = new int[soundCount]; // obf: fa.c
        TextEncoder.scratchStrings    = new String[soundCount]; // obf: h.e

        for (int i = 0; i < soundCount; i++) {
            EntityDef.modelNames[i] = ISAAC.readString();
        }
        for (int i = 0; i < soundCount; i++) {
            TextEncoder.scratchStrings[i] = ISAAC.readString();
        }
        for (int i = 0; i < soundCount; i++) {
            GameModel.sharedScratch[i] = ChatCipher.readNextBufferByte();
        }
        for (int i = 0; i < soundCount; i++) {
            ClientIOException.soundC[i] = ChatCipher.readNextBufferByte();
        }
        ...
        Packet.legacyBytes    = null; // obf: b.v = null
        InputState.nameDataBuffer = null; // obf: kb.d = null
    }
